...
const WeatherDetailss = () => {
  const {weatherData, isLoading} = useContext(WeatherContext);
  const [selectedCity, setSelectedDay] = useState(null)

  const handleDayClickk = async (day) => {
    setSelectedDay(day);
  ...
  const dailyForecast = weatherData.forecast.forecastday.map((day, index) => {
    const  dayOfWeek = new Date(day.date).toLocaleDateString
  })
}
